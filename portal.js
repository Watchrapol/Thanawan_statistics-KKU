...
async function showDashboard(user){
  ...
  // 1) ดึงข้อมูลนักศึกษาของอีเมลนี้
  const { data: me } = await sb
    .from('students')
    .select(`
      id, student_no, first_name, last_name, email,
      year_level, program, major, section,
      entry_year, avatar_url, kku_email
    `)
    .eq('email', user.email)
    .maybeSingle();
  ...
  if(!courseIds.length){
    $courses.innerHTML = `
      <div class="card"><h3 style="margin:0 0 8px">รายวิชาที่ลงทะเบียน</h3><div class="muted">ยังไม่มีรายวิชา</div></div>`;
    $score.innerHTML = '';
    return;
  }
        ...
        ${(courses||[]).map(c=>`
          <article class="card">
            ...
            <button class="btn btn--outline btn--sm"
              data-open-course data-course-id="${c.id}" data-course-code="${c.code}" data-student-id="${me.id}">
              ดูคะแนน
            </button>
          </article>
        `).join('')}
...
/* ===== การ์ดข้อมูลนักศึกษา (ไม่มีอาจารย์ที่ปรึกษา) + อัปโหลดรูปได้ ===== */
function initialsOf(me){
  const a = (me?.first_name||'').trim().charAt(0).toUpperCase();
  const b = (me?.last_name ||'').trim().charAt(0).toUpperCase();
  return (a||b) ? (a+b) : 'ID';
}
function renderWhoCard(me){
  ...
  $who.innerHTML = `
    <div class="card">
      ...
      <!-- ไม่มีรูป/ไม่มีอาจารย์ที่ปรึกษา -->
      <div>
        <div>
          <b>${me.first_name} ${me.last_name}</b>
          <span class="muted">(${me.student_no || '—'})</span>
        </div>
...
/* ===== เปิดการ์ดคะแนน (เหมือนเดิม + fallback เกรด) ===== */
async function openCourse(courseId, code, studentId){
  ...
  const rows = (assess||[]).map(a=>{
    ...
    return `
      <tr>
        ...
        <td class="score-cell right">${sc} / ${max}</td>
  ...
  // กติกาเกรดต่อวิชา (รองรับ 0–1 และ 0–100); ถ้าไม่มีใช้ fallback
  ...
  try{
    ...
    if(rules && typeof rules === 'object' && Object.keys(rules).length){
      rulePairs = Object.entries(rules);
      const maxCut = Math.max(...rulePairs.map(([,v])=>Number(v)));
      rulePairs = rulePairs.map(([g,v])=>[g, (maxCut<=1.001)? Number(v)*100 : Number(v)]);
    }else{
      rulePairs = Object.entries(fallbackRules);
    }
  }catch{ rulePairs = Object.entries(fallbackRules); }
  rulePairs.sort((a,b)=>b[1]-a[1]);
  const found  = rulePairs.find(([g,cut])=> totalPct >= cut);
        ...
        <table>
          ...
          <tfoot>
            <tr class="total-row">
              ...
              <td class="right"><b>${totalPct.toFixed(2)}%</b></td>
            </tr>
            <tr class="grade-row">
              <td colspan="3" align="right"><b>เกรด</b></td>
              <td class="right"><span class="pill pill--grade">${letter}</span></td>
            </tr>
          </tfoot>
        </table>
      </div>
...
function startResendCountdown(){
  let left=RESEND_WINDOW; btnSend.textContent=`ส่งใหม่ใน ${left}s`; btnSend.disabled=true;
  clearInterval(resendTimer);
  resendTimer=setInterval(()=>{
    left-=1; btnSend.textContent = left>0 ? `ส่งใหม่ใน ${left}s` : 'ส่งรหัส OTP';
...
function startOtpCountdown(){
  let left=OTP_WINDOW; otpCount.textContent=left; btnConfirm.textContent=`ยืนยัน (${left}s)`; btnConfirm.disabled=false;
  clearInterval(otpTimer);
  otpTimer=setInterval(()=>{
    left-=1; otpCount.textContent=left; btnConfirm.textContent=`ยืนยัน (${left}s)`;
    if(left<=0){
      clearInterval(otpTimer); btnConfirm.textContent='หมดเวลา'; btnConfirm.disabled=true;
